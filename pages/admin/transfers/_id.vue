    ...
    <div class="white" v-if="dynamicTransfers.length > 0">
      <v-row class="mx-0 px-0" justify="center">
        <VCol
          ...
          md="8"
          class="my-1 py-0"
          v-for="transfer in dynamicTransfers.slice(0, 50)"
          :key="transfer.id"
        >
        ...
        <VCol v-if="dynamicTransfers.length >= 50" cols="12" md="8">
          <VBtn
            color="accent"
            block
            large
            to="/admin/transfers/allTransfersList"
            >View All Transfers</VBtn
          >
        </VCol>
      </v-row>
    </div>
  ...
  computed: {
    ...
    dynamicTransfers() {
      const dynamicTransfers = {
        all: this.transfers,
        approved: this.approvedTransfers,
        pending: this.pendingTransfers,
        declined: this.declinedTransfers,
      }
      return dynamicTransfers[this.$route.params.id]
    },
